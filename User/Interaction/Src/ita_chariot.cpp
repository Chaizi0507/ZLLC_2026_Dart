...
void Class_Chariot::Control_Gimbal()
{
    ...
    if (tmp_gimbal_pitch > 0.0f)
        tmp_gimbal_pitch = 0.0f; // 这里是否应该是-10？
    if (tmp_gimbal_pitch < -25.0f)
        tmp_gimbal_pitch = -25.0f;
...
{
    // 角度目标值
    float tmp_booster_push, tmp_booster_pull;
    tmp_booster_push = Booster.Get_Target_PushMotor_Angle();
    tmp_booster_pull = Booster.Get_Target_PullMotor_Angle();

    static uint8_t booster_sign = 0;
    volatile int DR16_Left_Switch_Status = DR16.Get_Left_Switch();
    switch (DR16_Left_Switch_Status)
    {
    case (DR16_Switch_Status_MIDDLE): // 其余位置都是遥控器控制
    {
        //设定角度
        Booster.Set_Target_PullMotor_Angle(tmp_booster_pull);
        Booster.Set_Target_PushMotor_Angle(tmp_booster_push);
    }
    break;
    ...
    break;
    case (DR16_Switch_Status_UP): // 校准模式
    {
        Booster.Set_Booster_Control_Type(Booster_Control_Type_Push_CALIBRATION);
    }
    break;
    }
}
